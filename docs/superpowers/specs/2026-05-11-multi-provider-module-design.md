...
- **Mixed-runtime within one agent invocation.** A provider is chosen per agent per run; no mid-run switching.
- **Silent fallback between providers.** If the operator chose Gemini and Gemini fails, the agent fails. Fallback to another provider would hide quota leaks and break reproducibility.
