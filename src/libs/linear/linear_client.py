...
class LinearClient:
    ...
    def list_projects(self, team_id: str) -> list[Project]:
        ...
        return [
            Project.from_linear(linear_project=proj)
            for proj in linear_projects.values()
        ]
    ...
    def list_issues(self, project_id: str) -> list[Issue]:
        ...
        raw_issues = self._client.projects.get_issues(project_id=project_id)
        result: list[Issue] = []
        for raw in raw_issues:
            if isinstance(raw, dict):
                result.append(Issue.model_validate(raw))
            else:
                result.append(Issue.from_linear(raw))
        return result
    ...
    def create_issue(self, input_data: IssueInput) -> Issue:
        ...
        # linear_api resolves team/project by name, not ID — resolve first
        team = self.get_team(input_data.team_id)
        if team is None:
            raise ValueError(
                f"Linear team not found or inaccessible: {input_data.team_id!r}"
            )
        project = self.get_project(input_data.project_id)
        if project is None:
            raise ValueError(
                f"Linear project not found or inaccessible: {input_data.project_id!r}"
            )
    ...
    def update_issue(self, issue_id: str, input_data: IssueUpdateInput) -> Issue:
        ...
        Re-parents the issue when `input_data.parent_id` is provided (turning
        it into a sub-issue under the given parent).
        """
