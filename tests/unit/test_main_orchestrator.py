...
async def test_mode_routing_cascade():
    ...
    with (
        ...
        patch(
            "hsb.agents.main_orchestrator._cascade_dispatch", new_callable=AsyncMock
        ) as mock_cascade,
        patch(
            "hsb.agents.main_orchestrator._parallel_dispatch", new_callable=AsyncMock
        ) as mock_parallel,
        patch(
            "hsb.agents.main_orchestrator.run_validated_linear_agent",
            new_callable=AsyncMock,
        ),
    ):
        mock_go_instance = MockGO.return_value
        mock_go_instance.get_ready_tasks = AsyncMock(
...
async def test_cascade_sequential():
    ...
    with patch(
        "hsb.agents.main_orchestrator._run_wio_subprocess", new_callable=AsyncMock
    ) as mock_wio:
...
async def test_claiming_optimistic_lock():
    ...
    async def mock_linear(*args, **kwargs):
        ...
        if call_count[0] == 1:  # first read (pre-write)
            entity.get = lambda k, d=None: (
                "2024-01-01T00:00:00" if k == "updatedAt" else d
            )
        else:  # second read (post-write)
            entity.get = lambda k, d=None: (
                "2024-01-02T00:00:00" if k == "updatedAt" else d
            )
        return MagicMock(linear_entities=[entity])

    with patch(
        "hsb.agents.main_orchestrator.run_validated_linear_agent",
        side_effect=mock_linear,
    ):
...
def test_no_sdk_session_in_global_orchestrator():
    ...
    assert "claude_agent_sdk" not in source, (
        "D-01 violated: GlobalOrchestrator must be pure Python"
    )
    assert "ClaudeAgentOptions" not in source, (
        "D-01 violated: no SDK session in GlobalOrchestrator"
    )
...
def test_no_sdk_session_in_main_orchestrator():
    ...
    assert "claude_agent_sdk" not in source, (
        "D-02 violated: MainOrchestrator must be pure Python"
    )
    assert "ClaudeAgentOptions" not in source, (
        "D-02 violated: no SDK session in MainOrchestrator"
    )
    assert "**os.environ" not in source, (
        "T-4-04 violated: env var leakage to subprocess"
    )
    assert "shell=True" not in source, (
        "T-4-02 violated: shell=True allows command injection"
    )
