...
Create a pruned monorepo subset for Docker images. Outputs only the target package and its workspace dependencies.

```bash
turbo prune @ezra/app --docker
```

### `--docker` flag (recommended)

Splits output into two directories for optimal Docker layer caching:

- `out/json/` — `package.json` files + lockfile only (dependency layer)
- `out/full/` — full source code (build layer)

Without `--docker`, everything goes into `out/` as a single directory.

### Ezra's multi-stage Dockerfile pattern

Reference: `frontend/apps/app/Dockerfile`
...
WORKDIR /app
COPY . .
RUN turbo prune @ezra/app --docker

FROM base AS deps
WORKDIR /app
COPY --from=pruner /app/out/json/ ./    # Only package.json + lockfile
RUN pnpm install --frozen-lockfile       # Cached unless deps change

FROM deps AS builder
COPY --from=pruner /app/out/full/ ./    # Source code
RUN turbo run build --filter=@ezra/app
...
- `out/json/` layer changes only when dependencies change (great cache hit rate)
- `out/full/` layer changes on any source code change
- Requires `output: "standalone"` in `next.config.ts` for minimal runner image
- New apps should follow this same multi-stage pattern
- The `deps` stage is separate from `builder` so `pnpm install` is cached independently
