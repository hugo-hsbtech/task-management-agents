...
**Ezra-specific conventions** (see `frontend/CLAUDE.md` for full details):

- `@ezra/` namespace for all workspace packages
- JIT packages (no build step) — apps compile via `transpilePackages`
- Exact version pinning (no `^` or `~`)
- camelCase file names for `.ts`/`.tsx` source files
- `turbo.json` lives in `frontend/`, not repo root
